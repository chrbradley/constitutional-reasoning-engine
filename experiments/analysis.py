...
class ExperimentAnalyzer:
    ...
    def __init__(self, results_dir: Path = Path("results")):
        ...
        self.charts_dir = self.analysis_dir / "charts"

        # Create directories
        (self.analysis_dir / "single").mkdir(parents=True, exist_ok=True)
        (self.analysis_dir / "aggregate").mkdir(parents=True, exist_ok=True)
    ...
    def discover_experiments(self) -> List[str]:
        """Discover all experiment IDs in results/runs/"""
    ...
    def load_experiment_results(self, experiment_id: str) -> List[TestResult]:
        ...
        registry_path = self.runs_dir / experiment_id / "raw"
    ...
    def save_analysis(self, analysis: Dict[str, Any], filename: str):
        ...
        output_path = self.analysis_dir / "single" / filename

        # If multi-experiment, save to aggregate dir
        if analysis.get("analysis_type") == "multi_experiment":
            output_path = self.analysis_dir / "aggregate" / filename
